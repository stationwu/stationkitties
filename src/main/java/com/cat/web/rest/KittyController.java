...
//	private KittyService kittyService;
	...
	// @Transactional
	// public List<KittyContainer> addKittyToCustomer(HttpSession session)
	...
	// CustomerContainer customerContainer = new
	// CustomerContainer(customerRepository.save(customer));
	// return customerContainer.getKitties();
	// }

	@RequestMapping(path = MARKET_PATH, method = RequestMethod.GET)
	@Transactional
	public List<KittyContainer> getKittyMarket(Pageable pageRequest, HttpSession session)
			throws IOException, TranscoderException {
	...
	@Transactional
	public KittyContainer saleKitty(@Param("openCode") String openCode, @Param("kittyId") Long kittyId,
	...
	public String purchaseKitty(@Param("openCode") String openCode, @Param("kittyId") Long kittyId) throws Exception {
		...
		if (customer.getKitties().size() >= 9 && customer.getOpenCode() != "o3AIn0dgk_hseyPI3KuCooLx92bY") {
	...
	@Transactional
	public KittyContainer saleCancelKitty(@Param("openCode") String openCode, @Param("kittyId") Long kittyId)
			throws Exception {
		Kitty kitty = kittyRepository.findOne(kittyId);
		Customer customer = customerRepository.findOneByOpenCode(openCode);
		if (customer == null)
	...
	@Transactional
	public KittyContainer recoverlKitty(@Param("openCode") String openCode, @Param("kittyId") Long kittyId)
			throws Exception {
		Kitty kitty = kittyRepository.findOne(kittyId);
		Customer customer = customerRepository.findOneByOpenCode(openCode);
		if (customer == null)
